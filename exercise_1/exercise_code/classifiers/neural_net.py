...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        outputLayer1 = relu(np.dot(X, W1)+b1)

        #The output of the previous layer is the input of the following layer
        outputLayer2 = (np.dot(outputLayer1, W2) + b2)
        scores = outputLayer2
        ...
        #For info look at softmax.py and stability in file ipynb
        theta_xi = scores
        exp_theta_xi = np.exp(theta_xi)
        sum_exp_theta_xi = np.sum(exp_theta_xi, axis = 1)
        loss = np.log(sum_exp_theta_xi)
        loss = loss - theta_xi[range(N),y]
        loss = np.sum(loss) / float(N) + 0.5 * reg * (np.sum(W2*W2)+np.sum(W1*W1))
        ...
        # back propagation
        dout = np.copy((exp_theta_xi/(sum_exp_theta_xi.reshape(N,1))))  # (N, C)
        dout[np.arange(N), y] -= 1
        dz = np.dot(dout, W2.T) * ((np.dot(X, W1)+b1)> 0)

        # compute gradient for parameters
        grads['W2'] = np.dot(outputLayer1.transpose(), dout) / N
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            #Random sample at every iteration
            random_i = np.random.choice(num_train, batch_size)
            X_batch = X[random_i, :]
            y_batch = y[random_i]
    ...
    def predict(self, X):
        ...
        outputLayer1 = relu(np.dot(X, self.params["W1"]) + self.params["b1"])
        outputLayer2 = (np.dot(outputLayer1, self.params["W2"]) + self.params["b2"])
        y_pred = np.argmax(outputLayer2, axis=1)
